...
func getReplacementDefinitions() (defs []Definition) {
	defs = append(defs, Definition{
		...
		legacyEvalFn: func(this *Expression, es *EvalState) Ex {
			...
			if ok {
				newEx := ReplaceAll(this.Parts[1], rulesRule, &es.defined, &es.CASLogger, EmptyPD(), "")
			...
			if isList {
				...
				for i := 1; i < len(asList.Parts); i++ {
					...
					if ok {
						toReturn = ReplaceAll(toReturn.DeepCopy(), rulesRule, &es.defined, &es.CASLogger, EmptyPD(), "")
					}
				}
